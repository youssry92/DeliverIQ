...
export default function DeliverIQ(){
  ...
  const curTitle=isSet?titles[stab]:titles[view];

  return(<div style={{minHeight:"100vh",background:C.bg,color:C.tx,fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
    <style>{`*{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:${C.bd};border-radius:2px}button,select,input{font-family:inherit;outline:none}`}</style>
    ...
    <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(8,14,26,.9)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${C.bd}`,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:52}}>
      <div style={{display:"flex",alignItems:"center",gap:16}}>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <div style={{width:28,height:28,borderRadius:7,background:G,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:900,color:"#fff"}}>D</div>
          <div><div style={{fontSize:13,fontWeight:800,color:C.tx,letterSpacing:-.3,lineHeight:1}}>DeliverIQ</div><div style={{fontSize:6,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:C.ac,marginTop:1}}>Delivery Intelligence</div></div>
        </div>
        <div style={{display:"flex",gap:1,background:C.sf,borderRadius:8,padding:2}}>
          {NAV.map(n=><button key={n.id} onClick={()=>{sV(n.id);sS(false)}} style={{padding:"6px 14px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:600,background:!isSet&&view===n.id?C.bd:"transparent",color:!isSet&&view===n.id?C.tx:C.dm,transition:"all .15s",display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:10,opacity:!isSet&&view===n.id?1:.5}}>{n.i}</span>{n.l}</button>)}
          <button onClick={()=>sS(!isSet)} style={{padding:"6px 14px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,fontWeight:600,background:isSet?C.bd:"transparent",color:isSet?C.tx:C.dm,transition:"all .15s",display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:10,opacity:isSet?1:.5}}>⚙</span>Settings</button>
        </div>
      </div>
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        <span style={{fontSize:9,color:C.dm}}>Jun 24, 2026</span>
        <button style={{padding:"5px 12px",borderRadius:6,border:"none",fontSize:10,fontWeight:700,cursor:"pointer",background:G,color:"#fff"}}>Export</button>
      </div>
    </div>

    {/* Settings sub-tabs */}
    {isSet&&<div style={{padding:"0 24px",borderBottom:`1px solid ${C.bd}`,background:C.sf}}><div style={{display:"flex",gap:1,maxWidth:1200,margin:"0 auto"}}>{SNAV.map(s=><button key={s.id} onClick={()=>sSt(s.id)} style={{padding:"10px 16px",border:"none",cursor:"pointer",fontSize:11,fontWeight:600,background:"transparent",color:stab===s.id?C.ac:C.dm,borderBottom:`2px solid ${stab===s.id?C.ac:"transparent"}`,transition:"all .15s"}}>{s.l}</button>)}</div></div>}

    {/* CONTENT */}
    <div style={{padding:"20px 24px",maxWidth:1200,margin:"0 auto"}}>
      <div style={{marginBottom:16}}><div style={{fontSize:20,fontWeight:800,color:C.tx,letterSpacing:-.5}}>{curTitle}</div></div>
      {!isSet&&view==="portfolio"&&<Portfolio onSel={sP}/>}
      {!isSet&&view==="revenue"&&<Revenue/>}
      {!isSet&&view==="pms"&&<PMPerf onSel={sP}/>}
      {!isSet&&view==="ics"&&<ICPerf/>}
      {!isSet&&view==="mobile"&&<MobileConnect/>}
      {isSet&&stab==="challenges"&&<Challenges/>}
      {isSet&&stab==="mapper"&&<DataMapper/>}
      {isSet&&stab==="api"&&<APIConfig/>}
      {isSet&&stab==="sync"&&<SyncMon/>}
    </div>
